    ...
    <div className="space-y-6">
      <div className="card space-y-4">
        <h3 className="section-title flex items-center gap-2">
          <DollarSign className="w-4 h-4 text-emerald-700" /> {t('fobSummaryTitle')}
        </h3>
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-[#f0e8de] px-3 py-2.5">
            <p className="text-xs text-[#584531]">{t('usdLabel')}</p>
            <p className="text-base font-display font-bold text-[#3e2e1e]">{formatCurrency(data.amount_usd, 'USD')}</p>
          </div>
          <div className="bg-[#f0e8de] px-3 py-2.5">
            <p className="text-xs text-[#584531]">{t('exchangeRateLabel')}</p>
            <p className="text-base font-display font-bold text-[#3e2e1e]">R$ {data.exchange_rate.toFixed(2)}</p>
          </div>
          <div className="bg-[#584531]/10 border border-[#584531]/20 px-3 py-2.5">
            <p className="text-xs text-[#584531]">{t('brlLabel')}</p>
            <p className="text-base font-display font-bold text-[#3e2e1e]">{formatCurrency(data.amount_brl, 'BRL')}</p>
          </div>
        </div>
      </div>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div key={step.id} className={cn('flex gap-3 pb-4 last:pb-0 ml-4 pl-4', i < steps.length - 1 && 'border-l-2',
                done ? 'border-l-2 border-emerald-700/40 pl-4' : 'border-l-2 border-[#3e2e1e]/20 pl-4'
              )}>
                <div className={cn('w-8 h-8 flex items-center justify-center flex-shrink-0 -ml-8 border-2',
                  done ? 'bg-emerald-700/10 border-emerald-700' : active ? 'bg-[#584531]/10 border-[#584531] ring-2 ring-[#584531]/30 ring-offset-1 ring-offset-[#f0e8de]' : 'bg-[#f0e8de] border-[#584531]/30'
                )}>
                  {done ? <CheckCircle2 className="w-4 h-4 text-emerald-700" />
                    : active ? <DollarSign className="w-4 h-4 text-[#584531]" />
                    : <span className="text-xs text-[#584531]/60 font-bold">{i + 1}</span>}
                </div>
                <div className={cn('flex-1', i < steps.length - 1 ? 'pb-0' : '')}>
                  <p className={cn('text-sm font-medium', done ? 'text-emerald-700' : active ? 'text-[#584531]' : 'text-[#584531]/50')}>
                    {step.label}
                  </p>
                  <p className="text-xs text-[#584531]/60 mt-0.5">{step.desc}</p>

                  {/* Ação: BL */}
                  {step.id === 'SWIFT_RECEBIDO' && active && (
                    <div className="mt-3 space-y-2">
                      {data.swift_document_url ? (
                        <div className="flex items-center gap-2 bg-emerald-700/10 border border-emerald-700/30 px-3 py-2">
                          <FileCheck className="w-4 h-4 text-emerald-700" />
                          <span className="text-xs text-emerald-700">{t('swiftReceived')}</span>
                          <a href={data.swift_document_url} target="_blank" rel="noopener noreferrer" className="ml-auto text-xs text-[#584531] hover:underline flex items-center gap-1">
      ...
      <div className="space-y-3">
        {data.arrival_date && (
          <div className="flex items-center gap-2 bg-emerald-700/10 border border-emerald-700/30 px-3 py-2">
            ...
            <span className="text-xs text-emerald-700">{t('shipArrived', { date: data.arrival_date })}</span>
          </div>
        )}

        {data.payment_status === 'AGUARDANDO_CHEGADA' && (
          <div className="flex items-center gap-2 bg-[#584531]/10 border border-[#584531]/30 px-3 py-2">
            <Ship className="w-4 h-4 text-[#584531] animate-pulse-slow" />
            <span className="text-xs text-[#584531]">{t('inTransit')}</span>
          </div>
        )}
        ...
        {data.aflatoxin_result_at_destination && (
          <div className={cn('flex items-center gap-2 px-3 py-2 border',
            data.aflatoxin_result_at_destination === 'APROVADO'
              ? 'bg-emerald-700/10 border-emerald-700/30'
              : 'bg-red-500/10 border-red-500/30'
          )}>
            {data.aflatoxin_result_at_destination === 'APROVADO'
              ? <><CheckCircle2 className="w-4 h-4 text-emerald-600" /><span className="text-xs text-emerald-700">{t('aflatoxinApproved')}</span></>
              : <><AlertCircle className="w-4 h-4 text-red-500" /><span className="text-xs text-red-600">{t('aflatoxinRejected')}</span></>
            }
          </div>
        )}
        ...
        {data.payment_status === 'AGUARDANDO_SWIFT' && (
          <div className="flex items-center gap-2 bg-[#584531]/10 border border-[#584531]/30 px-3 py-2">
            <DollarSign className="w-4 h-4 text-[#584531]" />
            <span className="text-xs text-[#584531]">{t('awaitingSwiftCif')}</span>
          </div>
        )}
    ...
    return (
      <div className="p-6">
        ...
        <div className="card text-center py-12">
          <DollarSign className="w-10 h-10 mx-auto text-[#584531]/40 mb-2" />
          <p className="text-[#584531]">{t('noWorkflow')}</p>
        </div>
      </div>
    )
  }
