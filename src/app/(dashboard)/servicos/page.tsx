...
const typeVisual: Record<ServiceProviderType, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  CORRETORA:             { icon: DollarSign, color: 'text-emerald-400',bg: 'bg-emerald-400/10' },
  ...
  return (
    <div className="card hover:border-slate-600 transition-all flex flex-col gap-3">
      ...
      <div className="text-xs text-slate-500 space-y-0.5">
        {provider.coverage_area && <p>{t('coverage')} <span className="text-slate-300">{provider.coverage_area}</span></p>}
        {provider.ports_covered && <p>{t('ports')} <span className="text-slate-300">{provider.ports_covered.slice(0, 3).join(', ')}</span></p>}
        {provider.currency_pairs && <p>{t('currencies')} <span className="text-slate-300">{provider.currency_pairs.join(', ')}</span></p>}
        {provider.exchange_rate_usd && <p>{t('exchangeRate')} <span className="text-brand-300 font-semibold">R$ {provider.exchange_rate_usd.toFixed(2)}</span></p>}
        {provider.fixed_fee_brl && <p>{t('fixedFee')} <span className="text-slate-300">R$ {provider.fixed_fee_brl.toLocaleString('pt-BR')}</span></p>}
        {provider.mapa_accredited !== undefined && (
          provider.mapa_accredited
            ? <p className="flex items-center gap-1 text-emerald-400"><FileCheck className="w-3 h-3" /> {t('mapaAccredited')}</p>
